...
!function (t, e, i) {
    ...
    function s(t, i) {
        ...
        var s = i.processTemplate(d);
        return {restrict: "E", require: ["^?mdChips", "mdChip"], compile: n, controller: "MdChipCtrl"}
    }
    ...
    function r(t) {
        function e(e, i, n) {
            var s = e.$parent.$mdChipsCtrl, o = s.parent.$new(!1, s.parent);
            o.$$replacedScope = e, o.$chip = e.$chip, o.$index = e.$index, o.$mdChipsCtrl = s;
            var r = s.$scope.$eval(n.mdChipTransclude);
            i.html(r), t(i.contents())(o)
        }
    ...
    function h(t, e, i, n, s, o) {
        this.$timeout = s, this.$mdConstant = e, this.$scope = t, this.parent = t.$parent, this.$log = i, this.$element = n, this.ngModelCtrl = null, this.userInputNgModelCtrl = null, this.userInputElement = null, this.items = [], this.selectedChip = -1, this.hasAutocomplete = !1, this.enableChipEdit = o.parseAttributeBoolean(this.mdEnableChipEdit), this.deleteHint = "Press delete to remove this chip.", this.deleteButtonLabel = "Remove", this.chipBuffer = "", this.useTransformChip = !1, this.useOnAdd = !1, this.useOnRemove = !1
    }
    ...
    function p(t, e, i, n, s, o) {
        function r(r, h) {
            function c(t) {
                if (h.ngModel) {
                    var e = l[0].querySelector(t);
                    return e && e.outerHTML
                }
            }

            var l = h.$mdUserTemplate;
            h.$mdUserTemplate = null;
            var d = c("md-chips>md-chip-template"), a = e.prefixer().buildList("md-chip-remove").map(function (t) {
                return "md-chips>*[" + t + "]"
            }).join(","), m = c(a) || p.remove, u = d || p["default"], C = c("md-chips>md-autocomplete") || c("md-chips>input") || p.input, f = l.find("md-chip");
            return l[0].querySelector("md-chip-template>*[md-chip-remove]") && n.warn("invalid placement of md-chip-remove within md-chip-template."), function (n, r, c, l) {
                e.initOptionalProperties(n, h), t(r);
                var a = l[0];
                if (d && (a.enableChipEdit = !1), a.chipContentsTemplate = u, a.chipRemoveTemplate = m, a.chipInputTemplate = C, a.mdCloseIcon = o.mdClose, r.attr({
                        "aria-hidden": !0,
                        tabindex: -1
                    }).on("focus", function () {
                        a.onFocus()
                    }), h.ngModel && (a.configureNgModel(r.controller("ngModel")), c.mdTransformChip && a.useTransformChipExpression(), c.mdOnAppend && a.useOnAppendExpression(), c.mdOnAdd && a.useOnAddExpression(), c.mdOnRemove && a.useOnRemoveExpression(), c.mdOnSelect && a.useOnSelectExpression(), C != p.input && n.$watch("$mdChipsCtrl.readonly", function (t) {
                        t || e.nextTick(function () {
                            0 === C.indexOf("<md-autocomplete") && a.configureAutocomplete(r.find("md-autocomplete").controller("mdAutocomplete")), a.configureUserInput(r.find("input"))
                        })
                    }), e.nextTick(function () {
                        var t = r.find("input");
                        t && t.toggleClass("md-input", !0)
                    })), f.length > 0) {
                    var $ = i(f.clone())(n.$parent);
                    s(function () {
        ...
        function h() {
            return {
                chips: e.processTemplate(a),
                input: e.processTemplate(m),
                "default": e.processTemplate(u),
                remove: e.processTemplate(C)
            }
        }
    ...
    e.module("material.components.chips", ["material.core", "material.components.autocomplete"]), e.module("material.components.chips").controller("MdChipCtrl", n), n.$inject = ["$scope", "$element", "$mdConstant", "$timeout", "$mdUtil"], n.prototype.init = function (t) {
        ...
        var t = this.$element[0].getElementsByClassName("md-chip-content");
        ...
        if (this.isEditting) {
            ...
            var t = this.getChipIndex(), e = this.getContentElement().text();
            e ? (this.parentController.updateChipContents(t, this.getContentElement().text()), this.$mdUtil.nextTick(function () {
        ...
        var i, n;
        document.body.createTextRange ? (i = document.body.createTextRange(), i.moveToElementText(e), i.select()) : t.getSelection && (n = t.getSelection(), i = document.createRange(), i.selectNodeContents(e), n.removeAllRanges(), n.addRange(i))
    ...
    }, e.module("material.components.chips").directive("mdChip", s);
    var d = '    <span ng-if="!$mdChipsCtrl.readonly" class="md-visually-hidden">      {{$mdChipsCtrl.deleteHint}}    </span>';
    s.$inject = ["$mdTheming", "$mdUtil"], e.module("material.components.chips").directive("mdChipRemove", o), o.$inject = ["$timeout"], e.module("material.components.chips").directive("mdChipTransclude", r), r.$inject = ["$compile"], e.module("material.components.chips").controller("MdChipsCtrl", h), h.$inject = ["$scope", "$mdConstant", "$log", "$element", "$timeout", "$mdUtil"], h.prototype.inputKeydown = function (t) {
        var e = this.getChipBuffer();
        if (!(this.hasAutocomplete && t.isDefaultPrevented && t.isDefaultPrevented())) {
            if (t.keyCode === this.$mdConstant.KEY_CODE.BACKSPACE) {
                if (0 !== this.getCursorPosition(t.target))return;
                return t.preventDefault(), t.stopPropagation(), void(this.items.length && this.selectAndFocusChipSafe(this.items.length - 1))
            }
            if ((!this.separatorKeys || this.separatorKeys.length < 1) && (this.separatorKeys = [this.$mdConstant.KEY_CODE.ENTER]), this.separatorKeys.indexOf(t.keyCode) !== -1) {
                if (this.hasAutocomplete && this.requireMatch || !e)return;
                if (t.preventDefault(), this.hasMaxChipsReached())return;
                this.appendChip(e.trim()), this.resetChipBuffer()
            }
        }
        ...
        if (!this.getChipBuffer() && !this.isEditingChip())switch (t.keyCode) {
            case this.$mdConstant.KEY_CODE.BACKSPACE:
            case this.$mdConstant.KEY_CODE.DELETE:
                if (this.selectedChip < 0)return;
                if (t.preventDefault(), !this.isRemovable())return;
                this.removeAndSelectAdjacentChip(this.selectedChip);
                break;
            case this.$mdConstant.KEY_CODE.LEFT_ARROW:
                t.preventDefault(), this.selectedChip < 0 && (this.selectedChip = this.items.length), this.items.length && this.selectAndFocusChipSafe(this.selectedChip - 1);
                break;
            case this.$mdConstant.KEY_CODE.RIGHT_ARROW:
                t.preventDefault(), this.selectAndFocusChipSafe(this.selectedChip + 1);
                break;
        ...
        var t = this.items && this.items.length && ("" == this.secondaryPlaceholder || this.secondaryPlaceholder);
        return t ? this.secondaryPlaceholder : this.placeholder
    }, h.prototype.removeAndSelectAdjacentChip = function (t) {
        var i = this.getAdjacentChipIndex(t);
        this.removeChip(t), this.$timeout(e.bind(this, function () {
        ...
        if (this.useTransformChip && this.transformChip) {
            var i = this.transformChip({$chip: t});
            e.isDefined(i) && (t = i)
        }
        if (e.isObject(t)) {
        ...
        var e = this.items.splice(t, 1);
        this.ngModelCtrl.$setDirty(), this.validateModel(), e && e.length && this.useOnRemove && this.onRemove && this.onRemove({
            $chip: e[0],
            $index: t
        })
        ...
        var n = this.$scope, s = this, o = function (t, i) {
        ...
        t && (this.hasAutocomplete = !0, t.registerSelectedItemWatcher(e.bind(this, function (t) {
            if (t) {
    ...
    }, e.module("material.components.chips").directive("mdChips", p);
