...
function MdChip(t, e) {
    function i(i, s) {
        return i.append(e.processTemplate(n)), function (e, i, n, s) {
            var o = s.shift(), r = s.shift();
            t(i), o && (r.init(o), angular.element(i[0].querySelector(".md-chip-content")).on("blur", function () {
                o.resetSelectedChip(), o.$scope.$applyAsync()
            }))
        }
    }

    var n = e.processTemplate(DELETE_HINT_TEMPLATE);
...
function MdChipsCtrl(t, e, i, n, s, o) {
...
function MdChips(t, e, i, n, s, o) {
    function r(r, h) {
        function l(t) {
            if (h.ngModel) {
                var e = d[0].querySelector(t);
                return e && e.outerHTML
            }
        }

        var d = h.$mdUserTemplate;
        h.$mdUserTemplate = null;
        var c = l("md-chips>md-chip-template"), a = e.prefixer().buildList("md-chip-remove").map(function (t) {
            return "md-chips>*[" + t + "]"
        }).join(","), C = l(a) || p.remove, m = c || p["default"], u = l("md-chips>md-autocomplete") || l("md-chips>input") || p.input, f = d.find("md-chip");
        return d[0].querySelector("md-chip-template>*[md-chip-remove]") && n.warn("invalid placement of md-chip-remove within md-chip-template."), function (n, r, l, d) {
            e.initOptionalProperties(n, h), t(r);
            var a = d[0];
            if (c && (a.enableChipEdit = !1), a.chipContentsTemplate = m, a.chipRemoveTemplate = C, a.chipInputTemplate = u, a.mdCloseIcon = o.mdClose, r.attr({
                    "aria-hidden": !0,
                    tabindex: -1
                }).on("focus", function () {
                    a.onFocus()
                }), h.ngModel && (a.configureNgModel(r.controller("ngModel")), l.mdTransformChip && a.useTransformChipExpression(), l.mdOnAppend && a.useOnAppendExpression(), l.mdOnAdd && a.useOnAddExpression(), l.mdOnRemove && a.useOnRemoveExpression(), l.mdOnSelect && a.useOnSelectExpression(), u != p.input && n.$watch("$mdChipsCtrl.readonly", function (t) {
                    t || e.nextTick(function () {
                        0 === u.indexOf("<md-autocomplete") && a.configureAutocomplete(r.find("md-autocomplete").controller("mdAutocomplete")), a.configureUserInput(r.find("input"))
                    })
                }), e.nextTick(function () {
                    var t = r.find("input");
                    t && t.toggleClass("md-input", !0)
                })), f.length > 0) {
                var g = i(f.clone())(n.$parent);
                s(function () {
    ...
    function h() {
        return {
            chips: e.processTemplate(MD_CHIPS_TEMPLATE),
            input: e.processTemplate(CHIP_INPUT_TEMPLATE),
            "default": e.processTemplate(CHIP_DEFAULT_TEMPLATE),
            remove: e.processTemplate(CHIP_REMOVE_TEMPLATE)
        }
    }
...
goog.provide("ngmaterial.components.chips"), goog.require("ngmaterial.components.autocomplete"), goog.require("ngmaterial.core"), angular.module("material.components.chips", ["material.core", "material.components.autocomplete"]), angular.module("material.components.chips").controller("MdChipCtrl", MdChipCtrl), MdChipCtrl.$inject = ["$scope", "$element", "$mdConstant", "$timeout", "$mdUtil"], MdChipCtrl.prototype.init = function (t) {
...
MdChip.$inject = ["$mdTheming", "$mdUtil"], angular.module("material.components.chips").directive("mdChipRemove", MdChipRemove), MdChipRemove.$inject = ["$timeout"], angular.module("material.components.chips").directive("mdChipTransclude", MdChipTransclude), MdChipTransclude.$inject = ["$compile"], angular.module("material.components.chips").controller("MdChipsCtrl", MdChipsCtrl), MdChipsCtrl.$inject = ["$scope", "$mdConstant", "$log", "$element", "$timeout", "$mdUtil"], MdChipsCtrl.prototype.inputKeydown = function (t) {
    ...
    var e = this.getAdjacentChipIndex(t);
    this.removeChip(t), this.$timeout(angular.bind(this, function () {
    ...
    var e = t.controller("ngModel");
    e != this.ngModelCtrl && (this.userInputNgModelCtrl = e);
    var i = this.$scope, n = this, s = function (t, e) {
    ...
    t && (this.hasAutocomplete = !0, t.registerSelectedItemWatcher(angular.bind(this, function (t) {
        if (t) {
...
MdChips.$inject = ["$mdTheming", "$mdUtil", "$compile", "$log", "$timeout", "$$mdSvgRegistry"], angular.module("material.components.chips").controller("MdContactChipsCtrl", MdContactChipsCtrl), MdContactChipsCtrl.prototype.queryContact = function (t) {
    var e = this.contactQuery({$query: t});
    return this.filterSelected ? e.filter(angular.bind(this, this.filterSelectedContacts)) : e
...
MdContactChips.$inject = ["$mdTheming", "$mdUtil"], ngmaterial.components.chips = angular.module("material.components.chips");